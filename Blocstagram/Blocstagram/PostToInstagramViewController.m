...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = NSLocalizedString(@"Apply Filter", @"apply filter to view title");
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (sepiaFilter && randomFilter && whiteSpecks && darkScratches && minimumComponent && composite) {
            ...
            CIImage *sepiaPlusWhiteSpecksImage = [CIFilter filterWithName:@"CISourceOverCompositing" keysAndValues:
                                                 kCIInputImageKey, whiteSpecksImage, kCIInputBackgroundImageKey,
                                                 sepiaImage, nil].outputImage;
