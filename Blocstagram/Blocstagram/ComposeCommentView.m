...
//  Blocstagram
//
//  Created by Chad Clayton on 9/1/15.
...
- (void) setIsWritingComment:(BOOL)isWritingComment animated:(BOOL)animated {
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            [self layoutSubviews];
         */
        [UIView animateWithDuration:1.2 delay:0.1 usingSpringWithDamping:0.7 initialSpringVelocity:9.0 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
            [self layoutSubviews];
        } completion: nil];
    } else {
        [self layoutSubviews];
