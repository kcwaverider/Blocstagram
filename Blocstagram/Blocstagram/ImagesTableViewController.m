...
#pragma mark - KVO for MediaTableViewCell
...
- (void) observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (object == [DataSource sharedInstance] && [keyPath isEqualToString:@"mediaItems"]) {
        ...
        if (kindOfChange == NSKeyValueChangeSetting) {
            ...
            // We have an incremental change: insered, deleted, or replaced
            
            //Get a list of the indeces that changed
            NSIndexSet *indexSetOfChanges = change[NSKeyValueChangeIndexesKey];
            
            // #1 - Cpnvert this NSIndex to an NSArray of NSIndexPaths (which is whatthe table view animation methods require)
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    CGRect keyboardFramInScreenCoordinates = frameValue.CGRectValue;
    CGRect keyboardFrameInViewCoordinates = [self.navigationController.view convertRect:keyboardFramInScreenCoordinates fromView:nil];
    ...
    if (CGRectIntersectsRect(keyboardFrameInViewCoordinates, commentViewFrameInViewCoordinates)) {
        // The 2 frames intersect (the keyboard would block the view)
